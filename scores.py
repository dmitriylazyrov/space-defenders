import pygame.font
from spaceship import SpaceShip
from pygame.sprite import Group

class Scores():
    def __init__(self, screen, stats):
        self.screen = screen
        self.screen_rec = screen.get_rect()
        self.stats = stats
        self.text_color = (139, 195, 74)
        self.font = pygame.font.SysFont(None, 48)
        self.image_score()
        self.image_high_score()
        self.image_spaces()

    def image_score(self):
        self.score_img = self.font.render(str(self.stats.score), True, self.text_color, (0, 0, 0))
        self.score_rect = self.score_img.get_rect()
        self.score_rect.right = self.screen_rec.right - 40
        self.score_rect.top = 20

    def image_high_score(self):
        self.high_score_image = self.font.render(str(self.stats.high_score), True, self.text_color, (0, 0, 0))
        self.high_score_rect = self.high_score_image.get_rect()
        self.high_score_rect.centerx = self.screen_rec.centerx
        self.high_score_rect.top = self.screen_rec.top = 20

    def image_spaces(self):
        self.spaces = Group()
        for space_number in range(self.stats.space_left):
            spaceship = SpaceShip(self.screen)
            spaceship.rect.x = 15 + space_number * spaceship.rect.width
            spaceship.rect.y = 20
            self.spaces.add(spaceship)

    def show_score(self):
        self.screen.blit(self.score_img, self.score_rect)
        self.screen.blit(self.high_score_image, self.high_score_rect)
        self.spaces.draw(self.screen)